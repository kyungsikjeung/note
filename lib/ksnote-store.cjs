...
class KsNoteStore {
  ...
  migrateSchema() {
    ...
    db.run("CREATE TABLE IF NOT EXISTS ai_audit (id TEXT PRIMARY KEY, ts INTEGER NOT NULL, provider TEXT, mode TEXT, target TEXT, note_id TEXT, instruction TEXT, status TEXT, ops_total INTEGER, ops_applied INTEGER, revision_before INTEGER, error TEXT)");
    this.addColumn("revisions", "reason", "TEXT");
    ...
    db.run("CREATE INDEX IF NOT EXISTS idx_ai_audit_ts ON ai_audit(ts DESC)");
  ...
  /** Additive migration helper: adds a column only when the table lacks it. */
  addColumn(table, column, type) {
    const existing = this.rows("PRAGMA table_info(" + table + ")");
  ...
  listRevisions(noteId, options) {
    ...
    return this.rows("SELECT id,note_id,title,created_at,reason FROM revisions WHERE note_id=? ORDER BY id DESC LIMIT ? OFFSET ?", [noteId, limit, opts.offset || 0]);
  }

  getRevision(id) {
    return this.row("SELECT id,note_id,title,content,created_at,reason FROM revisions WHERE id=?", [id]);
  }
  ...
  /**
   * Explicit revision snapshot. AI edits call this BEFORE applying a patch so
   * every AI change stays recoverable, and the returned id is what links an AI
   * session entry to the note history ("AI 세션과 실제 노트 변경 이력이 연결된다").
   */
  async snapshotRevision(request) {
    const record = request || {};
    const noteId = String(record.noteId || "");
    if (!noteId) throw new Error("스냅샷할 노트를 찾지 못했습니다.");
    const createdAt = Date.now();
    const reason = String(record.reason || "manual");
    return this.withWriteLock(() => {
      this.db.run(
        "INSERT INTO revisions(note_id,title,content,created_at,reason) VALUES(?,?,?,?,?)",
        [noteId, record.title || "", String(record.content || ""), createdAt, reason],
      );
      this.db.run(
        "DELETE FROM revisions WHERE id IN (SELECT id FROM revisions r WHERE r.note_id=? AND (SELECT COUNT(*) FROM revisions newer WHERE newer.note_id=r.note_id AND newer.id>=r.id)>" + REVISIONS_PER_NOTE + ")",
        [noteId],
      );
      return { id: this.currentRevision(noteId), noteId, createdAt, reason };
    });
  }

  /* ---------------------------------------------------------------- AI 실행 감사 로그 */

  /** One row per AI run outcome; surfaced in 설정 > 보안 > AI 실행 감사 로그. */
  async appendAiAudit(entry) {
    const record = entry || {};
    const row = {
      id: "aa-" + Date.now().toString(36) + "-" + crypto.randomBytes(4).toString("hex"),
      ts: Number(record.ts) || Date.now(),
      provider: String(record.provider || ""),
      mode: String(record.mode || ""),
      target: String(record.target || ""),
      note_id: String(record.noteId || record.note_id || ""),
      instruction: String(record.instruction || "").slice(0, 2000),
      status: String(record.status || ""),
      ops_total: Number(record.opsTotal || record.ops_total || 0),
      ops_applied: Number(record.opsApplied || record.ops_applied || 0),
      revision_before: Number(record.revisionBefore || record.revision_before || 0),
      error: record.error ? String(record.error).slice(0, 1000) : null,
    };
    return this.withWriteLock(() => {
      this.db.run(
        "INSERT INTO ai_audit(id,ts,provider,mode,target,note_id,instruction,status,ops_total,ops_applied,revision_before,error) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)",
        [row.id, row.ts, row.provider, row.mode, row.target, row.note_id, row.instruction, row.status, row.ops_total, row.ops_applied, row.revision_before, row.error],
      );
      return row;
    });
  }
  ...
  listAiAudit(options) {
    ...
    return this.rows(
      "SELECT id,ts,provider,mode,target,note_id,instruction,status,ops_total,ops_applied,revision_before,error FROM ai_audit ORDER BY ts DESC, id DESC LIMIT ? OFFSET ?",
      [limit, opts.offset || 0],
    );
  }
